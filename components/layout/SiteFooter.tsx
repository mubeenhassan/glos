...
function inferSocialKind(platform?: string, url?: string): SocialKind {
  const compactPlatform = (platform || "")
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  ...
  if (
    compactPlatform === "x" ||
    compactPlatform === "xtwitter" ||
    compactPlatform === "twitterx"
  ) {
    return "x";
  }
...
export default function SiteFooter({ siteSettings }: SiteFooterProps) {
  ...
  const socialLinks = (siteSettings?.socialLinks ?? []).filter((item) =>
    Boolean(item.url)
  );
  const footerLogoUrl = mediaImageUrl(
    { image: siteSettings?.footerLogo || siteSettings?.logo },
    220
  );
  ...
  const showBrandColumn = Boolean(
    footerLogoUrl || footerTitle || footerDescription || socialLinks.length > 0
  );
  const showLinkColumns = footerLinkColumns.some((column) => {
    const title = column.title?.trim();
    const links = (column.links ?? []).filter((item) =>
      Boolean(item.label || item.internalLink?.title)
    );
    return Boolean(title || links.length > 0);
  });
  const showContactColumn = Boolean(
    siteSettings?.footerContactTitle ||
      footerAddress ||
      footerPhone ||
      footerEmail
  );
            ...
            <div className="footer-brand-col">
              {footerLogoUrl || footerTitle ? (
                <Link
                  href="/"
                  className={
                    footerLogoUrl ? "footer-logo-link" : "footer-wordmark"
                  }
                >
                  {footerLogoUrl ? (
                    <img
                      ...
                      src={footerLogoUrl}
                      alt={siteSettings?.footerLogo?.alt || footerTitle || ""}
                    />
                  ) : (
                    footerTitle
                  )}
              ...
              {footerDescription ? (
                <p className="footer-description !w-full max-w-[310px] md:!max-w-[260px]">
                  {footerDescription}
                </p>
              ) : null}
           ...
           {footerLinkColumns.map((column, columnIndex) => {
            ...
            return (
              <div
                ...
                key={
                  column._key || `${title || "footer-column"}-${columnIndex}`
                }
              >
                <FooterColumnTitle
                  title={title}
                ...
                />
                <div className="footer-links !mt-7">
                  {links.map((item, index) => (
                    <NavigationLink
                      key={
                        item._key ||
                        `${item.label || "footer-column-link"}-${index}`
                      }
                      item={item}
              ...
              <div className="footer-contact-list">
                {footerAddress ? (
                  <div className="footer-contact-item ">
                    <span
                      className="footer-icon-circle rounded-full 
              ...
              <div className="footer-legal-links">
                {footerLegalLinks.map((item, index) => (
                  <NavigationLink
                    key={
                      item._key ||
                      `${item.label || "footer-legal-link"}-${index}`
                    }
                    item={item}
